...
import { markerDB } from '../api';
import { DirectionResponse, Route } from '../interfaces/directions';
import { Parqueadero } from '../interfaces/markermodule';
...
export class MarkerMapService {
  ...
  constructor(private directionsApi: markerDB){}
  ...
  createMarkersDBFrom(markermodule: Parqueadero[], userLocation:[number,number]){


    //si el mapa no se ha cargado correctamente, se enviara una alerta al usuario 
    if ( !this.map) throw Error('Mapa no inicializado, regargue el sisito')
    ...
    for( const markerparq of markermodule){
      const [ lng, lat ] = markerparq.center;
      const popup = new Popup()
      .setHTML(`
      <h6>${ markerparq.text }</h6>
      <span>${ markerparq.parque_name }</span>
      `);
    ...
    //validacion de mapa
    if( markermodule.length ===0 ) return;
